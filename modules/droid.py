...
class Options:
    """Configuration options for DROID-SLAM."""
    ...
    beta: float = 0.6
    warmup: int = 8
    filter_thresh: float = 0.45
    keyframe_thresh: float = 2.5
    frontend_thresh: float = 27.0
    frontend_window: int = 40
    frontend_radius: int = 2
    frontend_nms: int = 1
    backend_thresh: float = 30.0
    ...
    # DEBA parameters
    deba_w_e: float = 0.3      # weight for DepthPro depth estimate
    deba_w_c: float = 0.7      # weight for motion-based depth
    deba_num_knots: int = 8    # number of knots for cubic spline fitting
    # AGBA parameters
    agba_residual_thresh: float = 0.5    # residual threshold for triggering global BA
    agba_max_edges: int = 5000           # maximum edges in sparse pose graph
    agba_chebyshev_thresh: float = 50.0  # Chebyshev distance threshold for candidates
...
def run(
    ...
    """Run DROID-SLAM with DEBA and AGBA."""
    ...
    dataset = RGBDStream(
        ...
        resize=(320 + 32 + 32, 240 + 16 + 16),
    )

    for t, (image, depth, intr) in tqdm(enumerate(dataset), desc='DROID-SLAM'):
        if t < setting.t0:
            continue
        ...
        if keyframes != keyframe_watcher:
            ...
            if setting.global_ba_frontend > 0 and keyframes >= np.min([3, setting.global_ba_frontend]):
                if keyframes % setting.global_ba_frontend == 0:
                    droid.backend(7)
                    print(f'global-ba-frontend: {keyframes}')
        print(f'Frame: {t}, keyframes: {keyframe_watcher}')

    # fill non-keyframe poses
    ...
    # save per-frame 4x4 pose matrices
