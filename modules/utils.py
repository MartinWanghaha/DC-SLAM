...
def sample_from_video(
    ...
    """Sample frames from a video file at the specified FPS."""
    ...
    print(f'Sampling video {video_path} with stride {sample_rate}')
...
def calibrate_camera(
    ...
    """Calibrate camera from chessboard or circle pattern images.

    Args:
        images: image directory, video file, or list of images
        pattern_type: 'chessboard' or 'circle'
        pattern_size: (rows, cols) of the calibration pattern
        square_size: size of each square/circle in mm
        image_limit: max number of frames to use from video

    Returns:
        (reprojection_error, intrinsic [fx,fy,cx,cy], distortion [k1,k2,p1,p2,k3])
    """
    assert pattern_type in ['chessboard', 'circle'], \
        f'pattern_type must be chessboard or circle, got {pattern_type}'
    ...
    if isinstance(images, (str, Path)):
        if Path(images).suffix in ['.mp4', '.avi']:
            ...
            raise ValueError(f'images must be a video file or directory, got {images}')
    ...
    print('Detecting chessboard' if pattern_type == 'chessboard' else 'Detecting circle pattern')
    ...
    print(f'Using {len(img_points)} images for calibration')

    ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(obj_points, img_points, size, None, None)

    print(f'Reprojection error: {ret:.4f}')
    print(f'Intrinsic: fx={mtx[0][0]:.2f}, fy={mtx[1][1]:.2f}, cx={mtx[0][2]:.2f}, cy={mtx[1][2]:.2f}')
    print(f'Distortion: k1={dist[0][0]:.4f}, k2={dist[0][1]:.4f}, p1={dist[0][2]:.4f}, p2={dist[0][3]:.4f}, k3={dist[0][4]:.4f}')
...
def trajectory_to_poses(
    ...
    """Convert trajectory (TUM format) to per-frame 4x4 pose matrices."""
...
def K_from_intr(
    ...
    """Build 3x3 camera matrix from intrinsic parameters."""
    if intr is None:
