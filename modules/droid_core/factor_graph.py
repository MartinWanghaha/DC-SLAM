...
class FactorGraph:
    ...
    def add_proximity_factors(self, t0=0, t1=0, rad=2, nms=2, beta=0.25, thresh=16.0, remove=False):
        ...
        t = self.video.counter.value
        # 生成帧索引范围（左闭右开区间）
        ix = torch.arange(t0, t)  # 起始索引 t0 到当前帧
        jx = torch.arange(t1, t)  # 起始索引 t1 到当前帧
        # 创建帧间配对网格
        ii, jj = torch.meshgrid(ix, jx) # 生成所有可能的(i,j)对
        ii = ii.reshape(-1) # 展平为1D张量
        jj = jj.reshape(-1)
        # 计算帧间几何距离（核心指标）
        d = self.video.distance(ii, jj, beta=beta)
        d[ii - rad < jj] = np.inf # 屏蔽邻近帧（基于半径rad），排除j在i半径rad内的帧对
        # print(d.shape)
        d[d > 100] = np.inf # 屏蔽过大距离
        # 获取现有约束信息（正常/失效/非活跃）
        ii1 = torch.cat([self.ii, self.ii_bad, self.ii_inac], 0)
        jj1 = torch.cat([self.jj, self.jj_bad, self.jj_inac], 0)
        for i, j in zip(ii1.cpu().numpy(), jj1.cpu().numpy()): # 非极大值抑制处理（避免重复约束）
            for di in range(-nms, nms+1):
                for dj in range(-nms, nms+1):  # 动态调整抑制范围（基于帧间隔）
        ...
        es = []
        for i in range(t0, t):
            if self.video.stereo: # 处理当前帧的自相关（立体视觉时）
                es.append((i, i))
                d[(i-t0)*(t-t1) + (i-t1)] = np.inf

            for j in range(max(i-rad-1,0), i):# 添加半径内帧对（双向）
                es.append((i,j))
                es.append((j,i)) # 屏蔽已添加的位置
                d[(i-t0)*(t-t1) + (j-t1)] = np.inf

        ix = torch.argsort(d) # 按距离排序处理，获取距离升序索引
        for k in ix: # 阈值筛选与数量限制
            if d[k].item() > thresh:  # 超出阈值则跳过
                continue

            if len(es) > self.max_factors: # 超过最大因子数则终止
                break
            # 获取候选帧对
            i = ii[k]
            j = jj[k]
            
            # bidirectional # 添加双向约束
            es.append((i, j))
            es.append((j, i))

            for di in range(-nms, nms+1): # 对周围区域进行非极大抑制
        ...
        self.add_factors(ii, jj, remove) # 添加最终选择的因子
